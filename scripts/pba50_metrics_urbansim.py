...
def calculate_urbansim_highlevelmetrics(
    ...
    # all households
    metrics_dict[runid, metric_id, "tot_households_2050", y2, dbp] = parcel_sum_df[
        "tothh_2050"
    ].sum()
    metrics_dict[runid, metric_id, "tot_households_2015", y1, dbp] = parcel_sum_df[
        "tothh_2015"
    ].sum()
    metrics_dict[runid, metric_id, "hh_growth_region", y_diff, dbp] = (
        parcel_sum_df["tothh_2050"].sum() / parcel_sum_df["tothh_2015"].sum()
    )
    for index, row in county_sum_df.iterrows():
        metrics_dict[
            runid, metric_id, "hh_growth_%s" % row["county"], y_diff, dbp
        ] = row["tothh_growth"]
    ...
    metrics_dict[runid, metric_id, "LIHH_share_2015", y1, dbp] = (
        parcel_sum_df["hhq1_2015"].sum() + parcel_sum_df["hhq2_2050"].sum()
    ...
    for index, row in county_sum_df.iterrows():
        metrics_dict[
            runid, metric_id, "LIHH_growth_%s" % row["county"], y_diff, dbp
        ] = row["LIHH_growth"]

    # all jobs
    metrics_dict[runid, metric_id, "tot_jobs_2050", y2, dbp] = parcel_sum_df[
        "totemp_2050"
    ].sum()
    metrics_dict[runid, metric_id, "tot_jobs_2015", y1, dbp] = parcel_sum_df[
        "totemp_2015"
    ].sum()
    ...
    for index, row in county_sum_df.iterrows():
        metrics_dict[
            runid, metric_id, "jobs_growth_%s" % row["county"], y_diff, dbp
        ] = row["totemp_growth"]
...
def calculate_Affordable2_deed_restricted_housing(
    runid, dbp, parcel_sum_df, metrics_dict
):

    metric_id = "A2"

    # totals for 2050 and 2015
    metrics_dict[runid, metric_id, "deed_restricted_total", y2, dbp] = parcel_sum_df[
        "deed_restricted_units_2050"
    ].sum()
    metrics_dict[runid, metric_id, "deed_restricted_total", y1, dbp] = parcel_sum_df[
        "deed_restricted_units_2015"
    ].sum()
    metrics_dict[runid, metric_id, "residential_units_total", y2, dbp] = parcel_sum_df[
        "residential_units_2050"
    ].sum()
    metrics_dict[runid, metric_id, "residential_units_total", y1, dbp] = parcel_sum_df[
        "residential_units_2015"
    ].sum()
    metrics_dict[runid, metric_id, "deed_restricted_HRA", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("HRA", na=False),
        "deed_restricted_units_2050",
    ].sum()
    metrics_dict[runid, metric_id, "deed_restricted_HRA", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("HRA", na=False),
        "deed_restricted_units_2015",
    ].sum()
    metrics_dict[
        runid, metric_id, "residential_units_HRA", y2, dbp
    ] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("HRA", na=False),
        "residential_units_2050",
    ].sum()
    metrics_dict[
        runid, metric_id, "residential_units_HRA", y1, dbp
    ] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("HRA", na=False),
        "residential_units_2015",
    ].sum()
    ...
    metrics_dict[
        runid, metric_id, "deed_restricted_pct_new_units_nonHRA", y_diff, dbp
    ] = (
        metrics_dict[runid, metric_id, "deed_restricted_nonHRA_diff", y_diff, dbp]
        / metrics_dict[runid, metric_id, "residential_units_nonHRA_diff", y_diff, dbp]
    )
    ...
    print("********************A2 Affordable********************")
    print(
        "DR pct of new units %s" % dbp,
        metrics_dict[runid, metric_id, "deed_restricted_pct_new_units", y_diff, dbp],
    )
    print(
        "DR pct of new units in HRAs %s" % dbp,
        metrics_dict[
            runid, metric_id, "deed_restricted_pct_new_units_HRA", y_diff, dbp
        ],
    )
    print(
        "DR pct of new units outside of HRAs %s" % dbp,
        metrics_dict[
            runid, metric_id, "deed_restricted_pct_new_units_nonHRA", y_diff, dbp
        ],
    )
...
def calculate_Diverse1_LIHHinHRAs(
    ...
    metrics_dict[runid, metric_id, "LIHH_inHRA", y2, dbp] = (
        parcel_sum_df.loc[
            parcel_sum_df["pba50chcat"].str.contains("HRA", na=False), "hhq1_2050"
        ].sum()
        + parcel_sum_df.loc[
            parcel_sum_df["pba50chcat"].str.contains("HRA", na=False), "hhq2_2050"
        ].sum()
    )
    metrics_dict[runid, metric_id, "LIHH_inHRA", y1, dbp] = (
        parcel_sum_df.loc[
            parcel_sum_df["pba50chcat"].str.contains("HRA", na=False), "hhq1_2015"
        ].sum()
        + parcel_sum_df.loc[
            parcel_sum_df["pba50chcat"].str.contains("HRA", na=False), "hhq2_2015"
        ].sum()
    )
    ...
    metrics_dict[runid, metric_id, "TotHH_inHRA", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("HRA", na=False), "tothh_2015"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_inHRA", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("HRA", na=False), "tothh_2050"
    ].sum()
    # Total HHs in DR Tracts, in 2015 and 2050
    metrics_dict[runid, metric_id, "TotHH_inDRTracts", y1, dbp] = tract_sum_df.loc[
        (tract_sum_df["DispRisk"] == 1), "tothh_2015"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_inDRTracts", y2, dbp] = tract_sum_df.loc[
        (tract_sum_df["DispRisk"] == 1), "tothh_2050"
    ].sum()
    # Total HHs in CoC Tracts, in 2015 and 2050
    metrics_dict[runid, metric_id, "TotHH_inCoCTracts", y1, dbp] = tract_sum_df.loc[
        (tract_sum_df["coc_flag_pba2050"] == 1), "tothh_2015"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_inCoCTracts", y2, dbp] = tract_sum_df.loc[
        (tract_sum_df["coc_flag_pba2050"] == 1), "tothh_2050"
    ].sum()
    # Total HHs in GGs, in 2015 and 2050
    metrics_dict[runid, metric_id, "TotHH_inGGs", y1, dbp] = GG_sum_df[
        "tothh_2015"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_inGGs", y2, dbp] = GG_sum_df[
        "tothh_2050"
    ].sum()
    # Total HHs in Transit Rich GGs, in 2015 and 2050
    GG_TRich_sum_df = GG_sum_df[GG_sum_df["Designation"] == "Transit-Rich"]
    metrics_dict[runid, metric_id, "TotHH_inTRichGGs", y1, dbp] = GG_TRich_sum_df[
        "tothh_2015"
    ].sum()
    metrics_dict[runid, metric_id, "TotHH_inTRichGGs", y2, dbp] = GG_TRich_sum_df[
        "tothh_2050"
    ].sum()
    ...
    metrics_dict[runid, metric_id, "Q1HH_shareofDRTracts", y1, dbp] = (
        tract_sum_df.loc[(tract_sum_df["DispRisk"] == 1), "hhq1_2015"].sum()
    ...
    metrics_dict[runid, metric_id, "Q1HH_shareofDRTracts", y2, dbp] = (
        tract_sum_df.loc[(tract_sum_df["DispRisk"] == 1), "hhq1_2050"].sum()
    ...
    metrics_dict[runid, metric_id, "Q1HH_shareofGGs", y1, dbp] = (
        GG_sum_df["hhq1_2015"].sum()
        / metrics_dict[runid, metric_id, "TotHH_inGGs", y1, dbp]
    )
    ...
    metrics_dict[runid, metric_id, "Q1HH_shareofGGs", y2, dbp] = (
        GG_sum_df["hhq1_2050"].sum()
        / metrics_dict[runid, metric_id, "TotHH_inGGs", y2, dbp]
    )

    metrics_dict[runid, metric_id, "Q1HH_shareofTRichGGs", y1, dbp] = (
        GG_TRich_sum_df["hhq1_2015"].sum()
        / metrics_dict[runid, metric_id, "TotHH_inTRichGGs", y1, dbp]
    )
    ...
    metrics_dict[runid, metric_id, "Q1HH_shareofTRichGGs", y2, dbp] = (
        GG_TRich_sum_df["hhq1_2050"].sum()
        / metrics_dict[runid, metric_id, "TotHH_inTRichGGs", y2, dbp]
    )
...
def calculate_Diverse2_LIHH_Displacement(
    ...
    metrics_dict[runid, metric_id, "LIHH_inDR", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("DR", na=False), "hhq1_2050"
    ].sum()
    metrics_dict[runid, metric_id, "LIHH_inDR", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("DR", na=False), "hhq1_2015"
    ].sum()
    ...
    print("********************D2 Diverse********************")
    print(
        "Total Number of LIHH in DR tracts in 2050",
        metrics_dict[runid, metric_id, "LIHH_inDR", y2, dbp],
    )
    print(
        "Number of LIHH in DR tracts in 2015",
        metrics_dict[runid, metric_id, "LIHH_inDR", y1, dbp],
    )
    print(
        "Number of LIHH in DR tracts in normalized",
        metrics_dict[runid, metric_id, "LIHH_inDR_normalized", y1, dbp],
    )
    ...
    metrics_dict[runid, metric_id, "Num_DRtracts_total", y1, dbp] = tract_sum_df.loc[
        (tract_sum_df["DispRisk"] == 1), "tract_id"
    ].nunique()
    metrics_dict[runid, metric_id, "Num_CoCtracts_total", y1, dbp] = tract_sum_df.loc[
        (tract_sum_df["coc_flag_pba2050"] == 1), "tract_id"
    ].nunique()

    # Calculating tracts that lost Households
    tract_sum_df["hhq1_pct_2015_normalized"] = (
        tract_sum_df["hhq1_2015"] / tract_sum_df["tothh_2015"] * normalize_factor_Q1Q2
    )
    tract_sum_df["hhq1_pct_2050"] = (
        tract_sum_df["hhq1_2050"] / tract_sum_df["tothh_2050"]
    )
    ...
    for i in [0, 10]:
        ...
        tract_sum_df["lost_hhq1_%dpct" % i] = tract_sum_df.apply(
            lambda row: 1
            if ((row["hhq1_pct_2050"] / row["hhq1_pct_2015_normalized"]) < j)
            else 0,
            axis=1,
        )
        ...
        metrics_dict[
            runid, metric_id, "Num_DRtracts_lostLIHH_%dpct" % i, y_diff, dbp
        ] = tract_sum_df.loc[
            (
                (tract_sum_df["DispRisk"] == 1)
                & (tract_sum_df["lost_hhq1_%dpct" % i] == 1)
            ),
            "tract_id",
        ].nunique()
        metrics_dict[
            runid, metric_id, "Pct_DRtracts_lostLIHH_%dpct" % i, y_diff, dbp
        ] = float(
            metrics_dict[
                runid, metric_id, "Num_DRtracts_lostLIHH_%dpct" % i, y_diff, dbp
            ]
        ...
        print(
            "Number of DR Tracts that lost LIHH from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Num_DRtracts_lostLIHH_%dpct" % i, y_diff, dbp
            ],
        )
        print(
            "Pct of DR Tracts that lost LIHH from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Pct_DRtracts_lostLIHH_%dpct" % i, y_diff, dbp
            ],
        )
        ...
        metrics_dict[
            runid, metric_id, "Num_CoCtracts_lostLIHH_%dpct" % i, y_diff, dbp
        ] = tract_sum_df.loc[
            (
        ...
        metrics_dict[
            runid, metric_id, "Pct_CoCtracts_lostLIHH_%dpct" % i, y_diff, dbp
        ] = float(
            metrics_dict[
                runid, metric_id, "Num_CoCtracts_lostLIHH_%dpct" % i, y_diff, dbp
            ]
        ...
        print(
            "Number of CoC Tracts that lost LIHH from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Num_CoCtracts_lostLIHH_%dpct" % i, y_diff, dbp
            ],
        )
        print(
            "Pct of CoC Tracts that lost LIHH from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Pct_CoCtracts_lostLIHH_%dpct" % i, y_diff, dbp
            ],
        )
    ...
    GG_sum_df["hhq1_pct_2015_normalized"] = (
        GG_sum_df["hhq1_pct_2015"] * normalize_factor_Q1Q2
    )
    GG_sum_df["hhq1_pct_2050"] = GG_sum_df["hhq1_2050"] / GG_sum_df["tothh_2050"]

    # Total number of GGs
    metrics_dict[runid, metric_id, "Num_GGs_total", y1, dbp] = GG_sum_df[
        "PDA_ID"
    ].nunique()
    # Total number of Transit Rich GGs
    GG_TRich_sum_df = GG_sum_df[GG_sum_df["Designation"] == "Transit-Rich"]
    metrics_dict[runid, metric_id, "Num_GGs_TRich_total", y1, dbp] = GG_TRich_sum_df[
        "PDA_ID"
    ].nunique()
    ...
    def check_GG_losthhq1(row, j):
        if row["hhq1_pct_2015_normalized"] == 0:
            return 0
        elif (row["hhq1_pct_2050"] / row["hhq1_pct_2015_normalized"]) < j:
            return 1
        else:
            return 0
    ...
    for i in [0, 10]:
        if i == 0:
            ...
            j = 0.9
        GG_sum_df["lost_hhq1_%dpct" % i] = GG_sum_df.apply(
            lambda row: check_GG_losthhq1(row, j), axis=1
        )
        GG_TRich_sum_df["lost_hhq1_%dpct" % i] = GG_TRich_sum_df.apply(
            lambda row: check_GG_losthhq1(row, j), axis=1
        )

        # Number or percent of GGs that lost Q1 households as a proportion of total HH
        metrics_dict[
            runid, metric_id, "Num_GG_lostLIHH_%dpct" % i, y_diff, dbp
        ] = GG_sum_df.loc[(GG_sum_df["lost_hhq1_%dpct" % i] == 1), "PDA_ID"].nunique()
        metrics_dict[
            runid, metric_id, "Pct_GG_lostLIHH_%dpct" % i, y_diff, dbp
        ] = float(
            metrics_dict[runid, metric_id, "Num_GG_lostLIHH_%dpct" % i, y_diff, dbp]
        ) / float(
            metrics_dict[runid, metric_id, "Num_GGs_total", y1, dbp]
        )
        print(
            "Number of GGs that lost LIHH from 2015 to 2050: ",
            metrics_dict[runid, metric_id, "Num_GG_lostLIHH_%dpct" % i, y_diff, dbp],
        )
        print(
            "Pct of GGs that lost LIHH from 2015 to 2050: ",
        ...
        metrics_dict[
            runid, metric_id, "Num_GG_TRich_lostLIHH_%dpct" % i, y_diff, dbp
        ] = GG_TRich_sum_df.loc[
            (GG_TRich_sum_df["lost_hhq1_%dpct" % i] == 1), "PDA_ID"
        ].nunique()
        metrics_dict[
            runid, metric_id, "Pct_GG_TRich_lostLIHH_%dpct" % i, y_diff, dbp
        ] = float(
            metrics_dict[
                runid, metric_id, "Num_GG_TRich_lostLIHH_%dpct" % i, y_diff, dbp
            ]
        ) / float(
            metrics_dict[runid, metric_id, "Num_GGs_TRich_total", y1, dbp]
        )
        print(
            "Number of Transit Rich GGs that lost LIHH from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Num_GG_TRich_lostLIHH_%dpct" % i, y_diff, dbp
            ],
        )
        print(
            "Pct of Transit Rich GGs that lost LIHH from 2015 to 2050: ",
            metrics_dict[
                runid, metric_id, "Pct_GG_TRich_lostLIHH_%dpct" % i, y_diff, dbp
            ],
        )
...
def calculate_Healthy1_HHs_SLRprotected(runid, dbp, parcel_sum_df, metrics_dict):
    ...
    metrics_dict[runid, metric_id, "SLR_protected_pct_affected_tothh", y2, dbp] = (
        tothh_2050_protected / tothh_2050_affected
    )
    metrics_dict[runid, metric_id, "SLR_protected_pct_affected_hhq1", y2, dbp] = (
        hhq1_2050_protected / hhq1_2050_affected
    )
    metrics_dict[runid, metric_id, "SLR_protected_pct_affected_CoChh", y2, dbp] = (
        CoChh_2050_protected / CoChh_2050_affected
    )

    print("********************H1 Healthy********************")
    print(
        "Pct of HHs affected by 3ft SLR that are protected in 2050 in %s" % dbp,
        metrics_dict[runid, metric_id, "SLR_protected_pct_affected_tothh", y2, dbp],
    )
    print(
        "Pct of Q1 HHs affected by 3ft SLR that are protected in 2050 in %s" % dbp,
        metrics_dict[runid, metric_id, "SLR_protected_pct_affected_hhq1", y2, dbp],
    )
    print(
        "Pct of CoC HHs affected by 3ft SLR that are protected in 2050 in %s" % dbp,
        metrics_dict[runid, metric_id, "SLR_protected_pct_affected_CoChh", y2, dbp],
    )
...
def calculate_Vibrant2_Jobs(runid, dbp, parcel_sum_df, metrics_dict):
    ...
    metrics_dict[runid, metric_id, "Total_jobs", y2, dbp] = parcel_sum_df[
        "totemp_2050"
    ].sum()
    metrics_dict[runid, metric_id, "Total_jobs", y1, dbp] = parcel_sum_df[
        "totemp_2015"
    ].sum()
    metrics_dict[runid, metric_id, "Total_jobs_growth", y_diff, dbp] = (
        metrics_dict[runid, metric_id, "Total_jobs", y2, dbp]
        / metrics_dict[runid, metric_id, "Total_jobs", y1, dbp]
        - 1
    )
    print(
        "Number of Jobs in 2050 %s" % dbp,
        metrics_dict[runid, metric_id, "Total_jobs", y2, dbp],
    )
    print(
        "Number of Jobs in 2015 %s" % dbp,
        metrics_dict[runid, metric_id, "Total_jobs", y1, dbp],
    )
    print(
        "Job Growth from 2015 to 2050 %s" % dbp,
        metrics_dict[runid, metric_id, "Total_jobs_growth", y_diff, dbp],
    )

    # MWTEMPN jobs
    metrics_dict[runid, metric_id, "Total_MWTEMPN_jobs", y2, dbp] = parcel_sum_df[
        "MWTEMPN_2050"
    ].sum()
    metrics_dict[runid, metric_id, "Total_MWTEMPN_jobs", y1, dbp] = parcel_sum_df[
        "MWTEMPN_2015"
    ].sum()
    metrics_dict[runid, metric_id, "Total_jobs_growth_MWTEMPN", y_diff, dbp] = (
        metrics_dict[runid, metric_id, "Total_MWTEMPN_jobs", y2, dbp]
        / metrics_dict[runid, metric_id, "Total_MWTEMPN_jobs", y1, dbp]
        - 1
    )
    print(
        "Number of Total MWTEMPN Jobs 2050 %s" % dbp,
        metrics_dict[runid, metric_id, "Total_MWTEMPN_jobs", y2, dbp],
    )
    print(
        "Number of Total MWTEMPN Jobs 2015 %s" % dbp,
        metrics_dict[runid, metric_id, "Total_MWTEMPN_jobs", y1, dbp],
    )
    print(
        "Job Growth Total MWTEMPN from 2015 to 2050 %s" % dbp,
        metrics_dict[runid, metric_id, "Total_jobs_growth_MWTEMPN", y_diff, dbp],
    )

    # Jobs Growth in PPAs

    metrics_dict[runid, metric_id, "PPA_jobs", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("ppa", na=False), "totemp_2050"
    ].sum()
    metrics_dict[runid, metric_id, "PPA_jobs", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("ppa", na=False), "totemp_2015"
    ].sum()
    metrics_dict[runid, metric_id, "jobs_growth_PPA", y_diff, dbp] = (
        metrics_dict[runid, metric_id, "PPA_jobs", y2, dbp]
        / metrics_dict[runid, metric_id, "PPA_jobs", y1, dbp]
        - 1
    )
    print(
        "Number of Jobs in PPAs 2050 %s" % dbp,
        metrics_dict[runid, metric_id, "PPA_jobs", y2, dbp],
    )
    print(
        "Number of Jobs in PPAs 2015 %s" % dbp,
        metrics_dict[runid, metric_id, "PPA_jobs", y1, dbp],
    )
    print(
        "Job Growth in PPAs from 2015 to 2050 %s" % dbp,
        metrics_dict[runid, metric_id, "jobs_growth_PPA", y_diff, dbp],
    )
    ...
    metrics_dict[runid, metric_id, "PPA_MWTEMPN_jobs", y2, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("ppa", na=False), "MWTEMPN_2050"
    ].sum()
    metrics_dict[runid, metric_id, "PPA_MWTEMPN_jobs", y1, dbp] = parcel_sum_df.loc[
        parcel_sum_df["pba50chcat"].str.contains("ppa", na=False), "MWTEMPN_2015"
    ].sum()
    metrics_dict[runid, metric_id, "jobs_growth_MWTEMPN_PPA", y_diff, dbp] = (
        metrics_dict[runid, metric_id, "PPA_MWTEMPN_jobs", y2, dbp]
        / metrics_dict[runid, metric_id, "PPA_MWTEMPN_jobs", y1, dbp]
        - 1
    )
    print(
        "Number of MWTEMPN Jobs in PPAs 2050 %s" % dbp,
        metrics_dict[runid, metric_id, "PPA_MWTEMPN_jobs", y2, dbp],
    )
    print(
        "Number of MWTEMPN Jobs in PPAs 2015 %s" % dbp,
        metrics_dict[runid, metric_id, "PPA_MWTEMPN_jobs", y1, dbp],
    )
    print(
        "Job Growth MWTEMPN in PPAs from 2015 to 2050 %s" % dbp,
        metrics_dict[runid, metric_id, "jobs_growth_MWTEMPN_PPA", y_diff, dbp],
    )
...
def calc_urbansim_metrics():
    ...
    for us_runid in list_us_runid:
        ...
        county_sum_df["tothh_growth"] = (
            county_sum_df["tothh_2050"] / county_sum_df["tothh_2015"]
        )
        county_sum_df["totemp_growth"] = (
            county_sum_df["totemp_2050"] / county_sum_df["totemp_2015"]
        )
        county_sum_df["LIHH_share_2050"] = (
            county_sum_df["hhq1_2050"] + county_sum_df["hhq2_2050"]
        ) / county_sum_df["tothh_2050"]
        county_sum_df["LIHH_share_2015"] = (
            county_sum_df["hhq1_2015"] + county_sum_df["hhq2_2015"]
        ) / county_sum_df["tothh_2015"]
        ...
        GG_sum_df = (
            parcel_sum_df.groupby(["Designation", "PDA_ID"])[
                "tothh_2050", "tothh_2015", "hhq1_2050", "hhq1_2015"
            ]
            .sum()
            .reset_index()
        ...
        GG_sum_df = GG_sum_df[
            (GG_sum_df["PDA_ID"] != "na") & (GG_sum_df["Designation"] != "Removed")
        ]
        GG_type_sum_df = (
            GG_sum_df.groupby(["Designation"])[
                "tothh_2050", "tothh_2015", "hhq1_2050", "hhq1_2015"
            ]
            .sum()
            .reset_index()
        ...
        calculate_urbansim_highlevelmetrics(
            us_runid, dbp, parcel_sum_df, county_sum_df, metrics_dict
        )
        calculate_Affordable2_deed_restricted_housing(
            us_runid, dbp, parcel_sum_df, metrics_dict
        )
...
if __name__ == "__main__":
    ...
    # Set UrbanSim inputs
    urbansim_run_location = "C:/Users/{}/Box/Modeling and Surveys/Urban Modeling/Bay Area UrbanSim 1.5/PBA50/Draft Blueprint runs/".format(
        os.getenv("USERNAME")
    )
    ...
    idx = pd.MultiIndex.from_tuples(
        metrics_dict.keys(), names=["modelrunID", "metric", "name", "year", "blueprint"]
    )
    metrics = pd.Series(metrics_dict, index=idx)
    metrics.name = "value"
    out_filename = "C:/Users/{}/Box/Horizon and Plan Bay Area 2050/Equity and Performance/7_Analysis/Metrics/metrics_urbansim.csv".format(
        os.getenv("USERNAME")
    )
