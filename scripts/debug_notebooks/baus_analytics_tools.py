...
def load_housing_data_from_parcel(parcel_file, res_cols):
    ...
    parcel_data = pd.read_csv(parcel_file,
                              usecols = ['parcel_id'] + res_cols)
    print('load {} rows of parcel data {}'.format(parcel_data.shape[0],
                                                  parcel_file))
    for col in res_cols:
...
def get_feasibility_by_useType(feasibility_df, dev_type,
                               keep_all_cols=True,
                               keep_only_allowed_parcels=False):
    """
    ...
    feasibility_df_reindex = feasibility_df.rename(columns={'Unnamed: 0_level_0': 'parcel_id',
                                                            'Unnamed: 0_level_1': 'parcel_id'})
    ...
    if keep_only_allowed_parcels:
        ...
        feasibility_devType = feasibility_devType.loc[feasibility_devType.max_profit.notnull()]
...
def compare_df_fields(df1, df2, common_id, compare_cols):
    if df1.shape[0] != df2.shape[0]:
        ...
        compare = df1.merge(df2,
                            left_on=common_id + '_1',
                            right_on=common_id + '_2',
                            how='outer')
...
def summarize_rsh_simulate_result_by_parcel(rsh_simulate_result_file,
                                            building_data_file):
    """
    Inputs:
    ...
    rsh_simulate_df = pd.read_csv(rsh_simulate_result_file,
                                  usecols = ['unit_id', 'unit_residential_price', 'building_id'])
    print('read {} rows of unit-level rsh_simulate result {}'.format(rsh_simulate_df.shape[0],
                                                                     rsh_simulate_result_file))

    # add 'parcel_id' field from building data
    building_data_df = pd.read_csv(building_data_file,
                                   usecols = ['building_id', 'parcel_id'])
    print('read {} rows of building_data {}'.format(building_data_df.shape[0],
                                                    building_data_file))

    rsh_simulate_df = rsh_simulate_df.merge(building_data_df, on='building_id', how='left')
    ...
    rsh_simulate_parcel_df = rsh_simulate_df.groupby(['parcel_id']).agg({
        'unit_residential_price': ['min', 'max', 'mean'],
        'unit_id': 'count'}).reset_index()
    rsh_simulate_parcel_df.columns = ['parcel_id',
                                      'unit_res_price_min', 'unit_res_price_max', 'unit_res_price_mean',
                                      'unit_cnt']
    return rsh_simulate_parcel_df

def summarize_rrh_simulate_result_by_parcel(rrh_simulate_result_file,
                                            building_data_file):
    """
    Inputs:
    ...
    # read rsh_simulate result
    rrh_simulate_df = pd.read_csv(rrh_simulate_result_file,
                                  usecols = ['unit_id', 'unit_residential_rent', 'building_id'])
    print('read {} rows of unit-level rrh_simulate result {}'.format(rrh_simulate_df.shape[0],
    ...
    rrh_simulate_df = rrh_simulate_df.merge(building_data_df, on='building_id', how='left')

    # get parcel-level unit_residential_rent values (max, min, median) and unit count
    # the model assumes all units of the same building have the same price
    rrh_simulate_parcel_df = rrh_simulate_df.groupby(['parcel_id']).agg({
        'unit_residential_rent': ['min', 'max', 'mean'],
        'unit_id': 'count'}).reset_index()
    rrh_simulate_parcel_df.columns = ['parcel_id',
...
def summarize_parcel_output_by_parcel(parcel_output_file, sum_cols):
    ...
    """
    parcel_output_df = pd.read_csv(parcel_output_file,
                                   usecols = ['parcel_id'] + sum_cols)
    print('read {} rows of parcel_output data {}, with {} unique parcel_id'.format(
        parcel_output_df.shape[0],
        parcel_output_file,
        len(parcel_output_df.parcel_id.unique())))

    parcel_output_parcel_df = parcel_output_df.groupby('parcel_id')[sum_cols].sum().reset_index()
...
def summarize_units_by_source_from_blg(building_file):
    ...
    building_df = pd.read_csv(building_file,
                              usecols=['building_id', 'parcel_id',
                                       'residential_units', 'unit_price', 'source'])
    print('read {} building_data {}, with {} unique parcel_id'.format(
        building_df.shape[0],
        building_file,
        len(building_df.parcel_id.unique())))
    ...
    df_dev.rename(columns = {'residential_units': 'resUnits_dev',
                             'unit_price'       : 'unit_price_dev'}, inplace=True)
    # aggregate by parcel_id
    df_parcel_dev = df_dev.groupby('parcel_id').agg({
        'resUnits_dev': 'sum',
        'unit_price_dev': 'mean'}).reset_index()
    ...
    df_nondev.rename(columns = {'residential_units': 'resUnits_nondev',
                                'unit_price'       : 'unit_price_nondev'}, inplace=True)
    df_parcel_nondev = df_nondev.groupby('parcel_id').agg({
        'resUnits_nondev': 'sum',
        'unit_price_nondev': 'mean'}).reset_index()
    df_parcel_nondev.columns = ['parcel_id', 'resUnits_nondev', 'unit_price_mean_nondev']
...
def compare_variables_between_scenarios(df, identifer, suffix_1, suffix_2,
                                        compare_cols, keep_cols=None):
    """
    ...
    for col in compare_cols:
        ...
        df_col_diff = df.loc[(df[col+'_'+suffix_1] != df[col+'_'+suffix_2]) \
        ...
        if df_col_diff.shape[0] > 0:
            ...
            #     suffix_1, suffix_2))
            # else:
            print ('compare {}: not identical between {} and {}'.format(
                col, suffix_1, suffix_2))
            diff_cols.append(col)
        else:
            print ('compare {}: identical between {} and {}'.format(
                col, suffix_1, suffix_2))

    if len(diff_cols) > 0:
        # keep only different cols and keep_cols if any
        df_diff_cols = df[[identifer] + keep_cols + sorted([x+'_'+suffix_1 for x in diff_cols] + \
