...
def simple_ipf(seed_matrix, col_marginals, row_marginals, tolerance=1, cnt=0):
    ...
    assert np.absolute(col_marginals - seed_matrix.sum(axis=0)).sum() < 0.01
    ...
    assert np.absolute(row_marginals - seed_matrix.sum(axis=1)).sum() < 0.01
    ...
    return simple_ipf(seed_matrix, col_marginals, row_marginals, tolerance, cnt + 1)
...
def compare_dfs(df1, df2):
    ...
    for label, row in df1.iterrows():
        ...
        for col, val in row.iteritems():
            ...
            df3.loc[label, col] = int(
                abs(val - val2) / ((val + val2 + 0.01) / 2.0) * 100.0
            )
...
def small_vals(df):
    ...
    for col in df.columns:
        df[col] = df[col] < df[col].mean() - 0.5 * df[col].std()
...
def compare_dfs_excel(df1, df2, excelname="out.xlsx"):
    ...
    reds = {i: workbook.add_format({"bg_color": reds[i]}) for i in range(len(reds))}

    blues = palettable.colorbrewer.sequential.Oranges_5.hex_colors
    blues = {i: workbook.add_format({"bg_color": blues[i]}) for i in range(len(blues))}
...
def ue_config(run_num, host):
    data = {
        "taz_url": (
            "https://landuse.s3.us-west-2.amazonaws.com/"
    ...
    r = requests.post(
        "https://forecast-feedback.firebaseio.com/simulations.json", json.dumps(data)
    )
...
def format_df(df, formatters=None, **kwargs):
    ...
    for col, formatter in formatters.items():
        try:
            df[col] = df[col].apply(
                lambda x: na_rep if pd.isnull(x) else formatter.format(x)
            )
        except KeyError:
            (
...
def compare_series(base_series, outcome_series, index):
    ...
    columns = ["Count", "Share", "Percent_Change", "Share_Change"]
...
def to_esri_csv(df, variable, runs):
    f = "compare/esri_" + "%(variable)s_%(runs)s.csv" % {
        "variable": variable,
...
def write_bundle_comparison_csv(df, variable, runs):
    ...
    f = "compare/" + "%(geography)s_%(variable)s_%(runs)s.csv" % {
        "geography": geography,
...
def write_csvs(df, variable, runs):
    f = "compare/" + "%(variable)s_%(runs)s.csv" % {
        "variable": variable,
        "runs": "-".join(str(x) for x in runs),
    }
...
def subtract_base_year_urban_footprint(run_number):
    base_year_filename = "runs/run{}_urban_footprint_summary_summaries_{}.csv".format(
        run_number, 2010
    )
    bdf = pd.read_csv(base_year_filename, index_col=0)
    outcome_year_filename = (
        "runs/run{}_urban_footprint_summary_summaries_{}.csv".format(run_number, 2040)
    )
    odf = pd.read_csv(outcome_year_filename, index_col=0)
    sdf = odf - bdf
    sdf.to_csv(
        "runs/run{}_urban_footprint_subtracted_summaries_{}.csv".format(
            run_number, 2040
        )
    )
