...
def profit_adjustment_strategies():
    with open(
        os.path.join(
            ...
            "plan_strategies/profit_adjustment_strategies.yaml",
        )
    ) as f:
        return yaml.load(f)
...
def account_strategies():
    with open(
        os.path.join(
            orca.get_injectable("inputs_dir"), "plan_strategies/account_strategies.yaml"
        )
    ) as f:
        return yaml.load(f)
...
def inclusionary_strategy():
    with open(
        os.path.join(
            orca.get_injectable("inputs_dir"),
            "plan_strategies/inclusionary_strategy.yaml",
        )
    ) as f:
        return yaml.load(f)
...
def preservation():
    with open(
        os.path.join(
            orca.get_injectable("inputs_dir"), "plan_strategies/preservation.yaml"
        )
    ) as f:
        return yaml.load(f)
...
def base_year_summary_taz(mapping):
    df = pd.read_csv(
        os.path.join("output", "baseyear_taz_summaries_2010.csv"),
        dtype={"taz1454": np.int64},
        index_col="zone_id",
    )
...
def zoning_lookup():
    ...
    file = os.path.join(
        orca.get_injectable("inputs_dir"),
        "basis_inputs/zoning/2020_11_05_zoning_lookup_hybrid_pba50.csv",
    )
...
def zoning_existing(parcels, zoning_lookup):
    ...
    file = os.path.join(
        orca.get_injectable("inputs_dir"),
        "basis_inputs/zoning/2020_11_05_zoning_parcels_hybrid_pba50.csv",
    )
...
def zoning_strategy(parcels_geography, mapping):
    ...
    strategy_zoning = pd.read_csv(
        os.path.join(
            orca.get_injectable("inputs_dir"), "plan_strategies/zoning_mods.csv"
        )
    )
...
def parcels_geography(parcels):
    ...
    file = os.path.join(
        orca.get_injectable("inputs_dir"),
        "basis_inputs/crosswalks/2021_02_25_parcels_geography.csv",
    )
    print("Versin of parcels_geography: {}".format(file))
    df = pd.read_csv(
        file,
        dtype={"PARCEL_ID": np.int64, "geom_id": np.int64, "jurisdiction_id": np.int64},
        index_col="geom_id",
    )
    df = geom_id_to_parcel_id(df, parcels)
    ...
    juris_name = pd.read_csv(
        os.path.join(
            orca.get_injectable("inputs_dir"),
            "basis_inputs/crosswalks/census_id_to_name.csv",
        ),
...
def mandatory_accessibility(year, run_setup):
    ...
    if year in run_setup["logsum_period1"]:
        df = pd.read_csv(
            os.path.join(
                orca.get_injectable("inputs_dir"),
                "accessibility/travel_model/mandatoryAccessibilities_{}.csv",
            ).format(run_setup["logsum_year1"])
        )
...
def non_mandatory_accessibility(year, run_setup):
    ...
    if year in run_setup["logsum_period1"]:
        df = pd.read_csv(
            os.path.join(
                orca.get_injectable("inputs_dir"),
                "accessibility/travel_model/nonMandatoryAccessibilities_{}.csv",
            ).format(run_setup["logsum_year1"])
        )
    elif year in run_setup["logsum_period2"]:
        df = pd.read_csv(
...
def accessibilities_segmentation(year, run_setup):
    ...
    if year in run_setup["logsum_period1"]:
        df = pd.read_csv(
            os.path.join(
                orca.get_injectable("inputs_dir"),
                "accessibility/travel_model/AccessibilityMarkets_{}.csv",
            ).format(run_setup["logsum_year1"])
        )
    elif year in run_setup["logsum_period2"]:
        df = pd.read_csv(
...
def household_controls_unstacked():
    return pd.read_csv(
        os.path.join(
            orca.get_injectable("inputs_dir"),
            "regional_controls/household_controls.csv",
        ),
        index_col="year",
    )
...
def employment_controls_unstacked():
    return pd.read_csv(
        os.path.join(
            orca.get_injectable("inputs_dir"),
            "regional_controls/employment_controls.csv",
        ),
        index_col="year",
    )
...
def tm1_tm2_regional_controls():
    return pd.read_csv(
        os.path.join(
            orca.get_injectable("inputs_dir"),
            "zone_forecasts/tm1_tm2_regional_controls.csv",
        ),
        index_col="year",
    )
...
def sqft_per_job_adjusters():
    return pd.read_csv(
        os.path.join(misc.configs_dir(), "adjusters/sqft_per_job_adjusters.csv"),
        index_col="number",
    )
...
def telecommute_sqft_per_job_adjusters():
    return pd.read_csv(
        os.path.join(
            orca.get_injectable("inputs_dir"),
            "plan_strategies/telecommute_sqft_per_job_adjusters.csv",
        ),
        index_col="number",
    )
...
def slr_progression():
    return pd.read_csv(
        os.path.join(
            orca.get_injectable("inputs_dir"),
            "basis_inputs/hazards/slr_progression.csv",
        )
    )
...
def slr_parcel_inundation():
    return pd.read_csv(
        os.path.join(
            orca.get_injectable("inputs_dir"),
            "basis_inputs/hazards/slr_parcel_inundation.csv",
        ),
        dtype={"parcel_id": np.int64},
        index_col="parcel_id",
    )
...
def employment_relocation_rates_adjusters():
    df = pd.read_csv(
        os.path.join(
            misc.configs_dir(), "adjusters/employment_relocation_rates_overwrites.csv"
        )
    )
    df = df.set_index("zone_id")
    return df
