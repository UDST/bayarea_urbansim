...
@orca.table(cache=True)
def zoning_lookup():
    return pd.read_csv(os.path.join(misc.data_dir(), "2020_06_15_zoning_lookup_hybrid_pba50.csv"),
                       index_col='id')
...
def zoning_baseline(parcels, zoning_lookup, settings):
    df = pd.read_csv(os.path.join(misc.data_dir(),
                     "2020_06_15_zoning_parcels_hybrid_pba50.csv"),
                     index_col="geom_id")
...
def get_dev_projects_table(scenario, parcels):
    ...
    current_dev_proj = ("2020_0617_1433_development_projects.csv")
