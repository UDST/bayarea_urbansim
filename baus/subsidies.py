...
def coffer(settings):
    ...
    for key, acct in list(settings["acct_settings"]["lump_sum_accounts"].items()):
...
def inclusionary_housing_revenue_reduction(feasibility, units):
    ...
    value_can_afford = {k: value_can_afford(v) for k, v in
                        list(monthly_affordable_payment.to_dict().items())}
