...
import orca_test
import pandana
from baus.postprocessing import (
    GEO_SUMMARY_LOADER,
    TWO_GEO_SUMMARY_LOADER,
    nontaz_calculator,
    taz_calculator,
    county_calculator,
    juris_to_county,
...
def environment_config(run_number, parcels, year):
    ...
    f = open(
        os.path.join(
            ...
            "run%d_env_configuration.log" % (run_number),
        ),
        "w",
    )
...
def topsheet(
    ...
    hh_by_subregion = misc.reindex(
        taz_geography.subregion, households.zone_id
    ).value_counts()
    ...
    hhincome_by_inpda = households_df.income.groupby(
        households_df.pda_id.notnull()
    ).mean()
    ...
    hhincome_by_insesit = households_df.income.groupby(
        households_df.sesit_id.notnull()
    ).mean()
    # round to nearest 100s
    hhincome_by_insesit = (hhincome_by_insesit / 100).round() * 100

    jobs_by_subregion = misc.reindex(
        taz_geography.subregion, jobs.zone_id
    ).value_counts()

    jobs_df = orca.merge_tables(
        "jobs", [parcels, buildings, jobs], columns=["pda_id", "tra_id"]
    )
    ...
    f = open(
        os.path.join(
            orca.get_injectable("outputs_dir"),
            "run%d_topsheet_%d.log" % (run_number, year),
        ),
        "w",
    )
    ...
    write(
        "Base year mean income by whether household is in tra:\n%s"
        % base_year_measures["hhincome_by_intra"]
    )
    write(
        "Forecast year mean income by whether household is in tra:\n%s"
        % hhincome_by_intra
    )
    write(
        "Base year mean income by whether household is in hra/dr:\n%s"
        % base_year_measures["hhincome_by_insesit"]
    )
    write(
        "Forecast year mean income by whether household is in hra/dr:\n%s"
        % hhincome_by_insesit
    )
    ...
    if summary.parcel_output is not None:
        ...
        write(
            "Current share of units which are greenfield development:\n%s"
    ...
    households_by_county = (
        households.zone_id.map(taz_geography.county).map(cmap).value_counts()
    )
...
def diagnostic_output(
    ...
    zones["non_residential_sqft"] = buildings.groupby(
        "zone_id"
    ...
    zones["retail_sqft"] = (
        buildings.query('general_type == "Retail"')
        .groupby("zone_id")
        .non_residential_sqft.sum()
    )
    zones["office_sqft"] = (
        buildings.query('general_type == "Office"')
        .groupby("zone_id")
        .non_residential_sqft.sum()
    )
    ...
    zones["building_count"] = (
        buildings.query('general_type == "Residential"').groupby("zone_id").size()
    )
    # this price is the max of the original unit vector belows
    zones["residential_price"] = (
        buildings.query('general_type == "Residential"')
        .groupby("zone_id")
        .residential_price.quantile()
    )
    # these two are the original unit prices averaged up to the building id
    ru = residential_units
    zones["unit_residential_price"] = ru.unit_residential_price.groupby(
        ru.zone_id
    ).quantile()
    zones["unit_residential_rent"] = ru.unit_residential_rent.groupby(
        ru.zone_id
    ).quantile()
    ...
    zones["unit_residential_price_>_rent"] = (
        zones.unit_residential_price > (zones.unit_residential_rent * 12 / cap_rate)
    ).astype("int")

    zones["retail_rent"] = (
        buildings[buildings.general_type == "Retail"]
        .groupby("zone_id")
        .non_residential_rent.quantile()
    )
    zones["office_rent"] = (
        buildings[buildings.general_type == "Office"]
        .groupby("zone_id")
        .non_residential_rent.quantile()
    )
    ...
    if "dropped_buildings" in orca.orca._TABLES:
        ...
        df.to_csv(
            os.path.join(
                orca.get_injectable("outputs_dir"), "run{}_dropped_buildings.csv"
            ).format(run_number)
        )
...
def geographic_summary(
    ...
    if year in [2010, 2015, 2020, 2025, 2030, 2035, 2040, 2045, 2050]:
        ...
        for geography in geographies:
            ...
            summary_table = pd.pivot_table(
                households_df, values=["persons"], index=[geography], aggfunc=[np.size]
            )
            ...
            summary_table["res_units"] = buildings_df.groupby(
                geography
            ).residential_units.sum()
            summary_table["sfdu"] = (
                buildings_df.query("building_type == 'HS' or building_type == 'HT'")
                .groupby(geography)
                .residential_units.sum()
            )
            summary_table["mfdu"] = (
                buildings_df.query("building_type == 'HM' or building_type == 'MR'")
                .groupby(geography)
                .residential_units.sum()
            )
            ...
            summary_table["agrempn"] = (
                jobs_df.query("empsix == 'AGREMPN'").groupby(geography).size()
            )
            summary_table["mwtempn"] = (
                jobs_df.query("empsix == 'MWTEMPN'").groupby(geography).size()
            )
            summary_table["retempn"] = (
                jobs_df.query("empsix == 'RETEMPN'").groupby(geography).size()
            )
            summary_table["fpsempn"] = (
                jobs_df.query("empsix == 'FPSEMPN'").groupby(geography).size()
            )
            summary_table["herempn"] = (
                jobs_df.query("empsix == 'HEREMPN'").groupby(geography).size()
            )
            summary_table["othempn"] = (
                jobs_df.query("empsix == 'OTHEMPN'").groupby(geography).size()
            )
            ...
            summary_table["non_residential_sqft"] = buildings_df.groupby(geography)[
                "non_residential_sqft"
            ].sum()
            ...
            summary_table["deed_restricted_units"] = buildings_df.groupby(
                geography
            ).deed_restricted_units.sum()
            summary_table["preserved_units"] = buildings_df.groupby(
                geography
            ).preserved_units.sum()
            summary_table["inclusionary_units"] = buildings_df.groupby(
                geography
            ).inclusionary_units.sum()
            summary_table["subsidized_units"] = buildings_df.groupby(
                geography
            ).subsidized_units.sum()

            # additional columns from parcel_output
            if parcel_output is not None:

                # columns re: affordable housing
                summary_table["inclusionary_revenue_reduction"] = parcel_output.groupby(
                    geography
            ...
            if base is False:
                summary_csv = os.path.join(
                    orca.get_injectable("outputs_dir"), "run{}_{}_summaries_{}.csv"
    ...
    if year == final_year and (
        run_setup["run_housing_bond_strategy"]
        or run_setup["run_office_bond_strategy"]
        or run_setup["run_vmt_fee_strategy"]
        or run_setup["run_jobs_housing_fee_strategy"]
    ):

        for acct_name, acct in orca.get_injectable("coffer").items():
            fname = os.path.join(
                orca.get_injectable("outputs_dir"), "run{}_acctlog_{}_{}.csv"
    ...
    if year == final_year:
        ...
        for geography in geographies:
            df_base = pd.read_csv(
                os.path.join(
                    orca.get_injectable("outputs_dir"),
                    "run{}_{}_summaries_{}.csv".format(run_number, geography, baseyear),
                )
            )
            df_final = pd.read_csv(
                os.path.join(
                    orca.get_injectable("outputs_dir"),
                    "run{}_{}_summaries_{}.csv".format(
            ...
            df_growth.to_csv(
                os.path.join(
                    orca.get_injectable("outputs_dir"),
                    "run{}_{}_growth_summaries.csv".format(run_number, geography),
                ),
                index=False,
            )
    ...
    if year in [2010, 2015, 2020, 2025, 2030, 2035, 2040, 2045, 2050]:
        ...
        df = (
            buildings_uf_df.loc[buildings_uf_df["year_built"] > 2010]
            .groupby("urbanized")
            .sum()
        )
        df = df[["count", "residential_units", "non_residential_sqft", "acres"]]

        # where buildings were built after the base year,
        # sum if it was denser greenfield
        df2 = (
            buildings_uf_df.loc[buildings_uf_df["year_built"] > 2010]
            .groupby("denser_greenfield")
            .sum()
        )
        df2 = df2[["count", "residential_units", "non_residential_sqft", "acres"]]
        ...
        uf_summary_csv = os.path.join(
            orca.get_injectable("outputs_dir"),
            "run{}_urban_footprint_summary_{}.csv".format(run_number, year),
        )
    ...
    if year in [2010, 2015, 2020, 2025, 2030, 2035, 2040, 2045, 2050]:
        ...
        df.to_csv(
            os.path.join(
                orca.get_injectable("outputs_dir"),
                "run%d_taz_logsums_%d.csv" % (run_number, year),
            )
        )
        ...
        df.to_csv(
            os.path.join(
                orca.get_injectable("outputs_dir"),
                "run%d_parcel_logsums_%d.csv" % (run_number, year),
            )
        )
...
def parcel_summary(
    ...
    df = parcels.to_frame(
        ["geom_id", "x", "y", "total_job_spaces", "first_building_type"]
    )

    df2 = parcels_zoning_calculations.to_frame(["zoned_du", "zoned_du_underbuild"])
    ...
    df["inclusionary_units"] = building_df.groupby("parcel_id")[
        "inclusionary_units"
    ].sum()
    df["subsidized_units"] = building_df.groupby("parcel_id")["subsidized_units"].sum()

    jobs_df = orca.merge_tables(
        "jobs", [buildings, jobs], columns=["parcel_id", "empsix"]
    )
    ...
    df.to_csv(
        os.path.join(
            orca.get_injectable("outputs_dir"),
            "run%d_parcel_data_%d.csv" % (run_number, year),
        )
    )
    ...
    if year not in [2010, 2015]:
        ...
        df2 = pd.read_csv(
            os.path.join(
                orca.get_injectable("outputs_dir"),
                "run%d_parcel_data_%d.csv" % (run_number, initial_year),
            ),
            index_col="parcel_id",
        )
        ...
        df.to_csv(
            os.path.join(
                orca.get_injectable("outputs_dir"),
                "run%d_parcel_data_diff.csv" % run_number,
            )
        )
    ...
    if year not in [2010, 2015]:
        ...
        df_base = pd.read_csv(
            os.path.join(
                orca.get_injectable("outputs_dir"),
                "run%d_parcel_data_%d.csv" % (run_number, baseyear),
            )
        )
        df_final = pd.read_csv(
            os.path.join(
                orca.get_injectable("outputs_dir"),
                "run%d_parcel_data_%d.csv"  # % (run_number, final_year)))
                % (run_number, year),
            )
        )
        ...
        for geography in geographies:
            ...
            df_growth.sort_values(
                by=["county", "juris", "geo_category"],
                ascending=[True, True, False],
                inplace=True,
            )
            df_growth.set_index(
                ["RUNID", "county", "juris", "geo_category"], inplace=True
            )
            df_growth.to_csv(
                os.path.join(
                    orca.get_injectable("outputs_dir"),
        ...
        df_growth_1 = TWO_GEO_SUMMARY_LOADER(
            run_number, geo_1, geo_2, df_base, df_final
        )
        df_growth_1["county"] = df_growth_1["juris"].map(juris_to_county)
        df_growth_1.sort_values(
            by=["county", "juris", "geo_category"],
            ascending=[True, True, False],
            inplace=True,
        )
        df_growth_1.set_index(
            ["RUNID", "county", "juris", "geo_category"], inplace=True
        )
        df_growth_1.to_csv(
            os.path.join(
                orca.get_injectable("outputs_dir"),
                "run{}_{}_growth_summaries.csv".format(run_number, geo_1 + geo_2),
            )
        )

        df_growth_2 = TWO_GEO_SUMMARY_LOADER(
            run_number, geo_1, geo_3, df_base, df_final
        )
        df_growth_2["county"] = df_growth_2["juris"].map(juris_to_county)
        df_growth_2.sort_values(
            by=["county", "juris", "geo_category"],
            ascending=[True, True, False],
            inplace=True,
        )
        df_growth_2.set_index(
            ["RUNID", "county", "juris", "geo_category"], inplace=True
        )
        df_growth_2.to_csv(
            os.path.join(
...
def travel_model_output(
    ...
    summary,
    final_year,
    tm1_taz1_forecast_inputs,
    run_number,
    base_year_summary_taz,
    taz_geography,
    tm1_tm2_maz_forecast_inputs,
    tm1_tm2_regional_demographic_forecast,
    ...
    jobs_df = orca.merge_tables(
        "jobs", [parcels, buildings, jobs], columns=["zone_id", "zone_id_x", "empsix"]
    )
    ...
    taz_df["areatype"] = pd.cut(
        taz_df.density, bins=[0, 6, 30, 55, 100, 300, np.inf], labels=[5, 4, 3, 2, 1, 0]
    )
    ...
    taz_df.fillna(0).to_csv(
        os.path.join(
            orca.get_injectable("outputs_dir"), "run{}_taz_summaries_{}.csv"
    ...
    county_df["DENSITY"] = (
        county_df.TOTPOP + (2.5 * county_df.TOTEMP)
    ) / county_df.TOTACRE
    ...
    base_year_summary_taz = base_year_summary_taz.to_frame()
    base_year_summary_county = base_year_summary_taz.groupby("COUNTY_NAME").sum()
    base_year_summary_county_ciacre = base_year_summary_county["CIACRE_UNWEIGHTED"]
    base_year_summary_county_resacre = base_year_summary_county["RESACRE_UNWEIGHTED"]
    ...
    county_df.fillna(0).to_csv(
        os.path.join(
            orca.get_injectable("outputs_dir"), "run{}_county_summaries_{}.csv"
        ).format(run_number, year)
    )
    ...
    if year == final_year:
        ...
        df_growth = taz_calculator(run_number, df_base, df_final)
        df_growth = df_growth.set_index(
            ["RUNID", "TAZ", "SD", "SD_NAME", "COUNTY", "CNTY_NAME"]
        )
        df_growth.to_csv(
            os.path.join(
                orca.get_injectable("outputs_dir"),
        ...
        df_growth_c = county_calculator(run_number, df_base, df_final)
        df_growth_c.to_csv(
            os.path.join(
                orca.get_injectable("outputs_dir"),
                "run%d_county_growth_summaries.csv" % run_number,
            ),
            index=False,
        )
    ...
    pd.DataFrame(data={"REGION": [1], "gq_num_hh_region": [tot_gqpop]}).to_csv(
        os.path.join(
            orca.get_injectable("outputs_dir"), "run{}_regional_marginals_{}.csv"
        ).format(run_number, year),
        index=False,
    )
...
def travel_model_2_output(
    ...
    maz,
    year,
    tm2_emp27_employment_shares,
    run_number,
    tm1_tm2_maz_forecast_inputs,
    tm2_taz2_forecast_inputs,
    ...
    hh_df = orca.merge_tables(
        "households",
        [parcels, buildings, households],
        columns=["zone_id", "base_income_quartile", "income", "persons", "maz_id"],
    )
    ...
    jobs_df = orca.merge_tables(
        "jobs", [parcels, buildings, jobs], columns=["maz_id", "empsix"]
    )

    bldg_df = orca.merge_tables(
        "buildings", [buildings, parcels], columns=["maz_id", "residential_units"]
    )
    ...
    def getsectorcounts(empsix, empsh):
        ...
        return (
            emp
            * tm2_emp27_employment_shares.loc[
                tm2_emp27_employment_shares.empsh == empsh, str(year)
            ].values[0]
        )
    ...
    for i, r in maz.iterrows():
        c = r[emp_cols]
        maz.loc[i, emp_cols] = round_series_match_target(r[emp_cols], r.emp_total, 0)
    ...
    maz["ACRES"] = pcl.groupby("maz_id").acres.sum()
    maz["residential_units"] = bldg_df.groupby("maz_id").residential_units.sum()
    ...
    taz2["pop"] = (
        taz2.pop_hhsize1 + taz2.pop_hhsize2 + taz2.pop_hhsize3 + taz2.pop_hhsize4
    )
    ...
    county[["hh_wrks_1", "hh_wrks_2", "hh_wrks_3_plus"]] = taz2.groupby(
        "county_name"
    ).agg({"hh_wrks_1": "sum", "hh_wrks_2": "sum", "hh_wrks_3_plus": "sum"})

    county["workers"] = (
        county.hh_wrks_1 + county.hh_wrks_2 * 2 + county.hh_wrks_3_plus * 3.474036
    )
    ...
    county["pers_occ_management"] = round_series_match_target(
        county["pers_occ_management"], np.round(county["pers_occ_management"].sum()), 0
    )
    ...
    maz[
        ...
        os.path.join(
            orca.get_injectable("outputs_dir"),
            "run{}_maz_summaries_{}.csv".format(run_number, year),
        )
    )
    ...
    maz[
        ...
        os.path.join(
            orca.get_injectable("outputs_dir"),
            "run{}_maz_marginals_{}.csv".format(run_number, year),
        )
    )
    ...
    taz2[
        ...
        os.path.join(
            orca.get_injectable("outputs_dir"),
            "run{}_taz2_marginals_{}.csv".format(run_number, year),
        )
    )
    ...
    county[
        ...
        os.path.join(
            orca.get_injectable("outputs_dir"),
            "run{}_county_marginals_{}.csv".format(run_number, year),
        )
    )
...
def add_population_tm2(df, year, tm1_tm2_regional_controls):
    ...
    target = rc.totpop.loc[year] - df.gqpop.sum()
    s = df.hhpop
    s = scale_by_target(s, target, 0.15)
    df["hhpop"] = round_series_match_target(s, target, 0)
    df["hhpop"] = df.hhpop.fillna(0)
    return df
...
def add_employment(df, year, tm1_tm2_regional_controls):
    ...
    empshare = (
        0.46381 * hh_shares.hhincq1
        + 0.49361 * hh_shares.hhincq2
        + 0.56938 * hh_shares.hhincq3
        + 0.29818 * hh_shares.hhincq4
    ...
    empshare = empshare.fillna(0).clip(0.3, 0.7)
...
def add_age_categories(df, year, tm1_tm2_regional_controls):
    ...
    seed_matrix = (
        zfi[["sh_age0004", "sh_age0519", "sh_age2044", "sh_age4564", "sh_age65p"]]
        .mul(df.totpop, axis="index")
        .as_matrix()
    )
    ...
    col_marginals = (
        scale_by_target(pd.Series(col_marginals), target).round().astype("int")
    )

    seed_matrix[seed_matrix == 0] = 0.1
...
def adjust_hhsize(df, year, rdf, total_hh):
    col_marginals = (
        rdf.loc[rdf.year == year, ["shrs1", "shrs2", "shrs3", "shrs4"]] * total_hh
    ).values[0]
    row_marginals = df.hh.fillna(0).values
    seed_matrix = np.round(
        df[["hh_size_1", "hh_size_2", "hh_size_3", "hh_size_4_plus"]]
    ).as_matrix()
...
def adjust_hhwkrs(df, year, rdf, total_hh):
    col_marginals = (
        rdf.loc[rdf.year == year, ["shrw0", "shrw1", "shrw2", "shrw3"]] * total_hh
    ).values[0]
    row_marginals = df.hh.fillna(0).values
    seed_matrix = np.round(
        df[["hh_wrks_0", "hh_wrks_1", "hh_wrks_2", "hh_wrks_3_plus"]]
    ).as_matrix()
...
def adjust_page(df, year, tm1_tm2_regional_controls):
    ...
    seed_matrix = np.round(
        df[["pers_age_00_19", "pers_age_20_34", "pers_age_35_64", "pers_age_65_plus"]]
    ).as_matrix()
    ...
    if run_setup["run_slr"]:
        ...
        if len(destroy_parcels) > 0:
            ...
            f = open(
                os.path.join(
                    orca.get_injectable("outputs_dir"),
                    "run%d_hazards_slr_%d.log" % (run_number, year),
                ),
                "w",
            )
            ...
            # income quartile counts
            try:
                hh_unplaced_slr_cum = orca.get_table("hh_unplaced_slr_cum").to_frame()
            except Exception as e:
            ...
            # employees by sector
            try:
                jobs_unplaced_slr_cum = orca.get_table(
            ...
            slr_demolish.to_csv(
                os.path.join(
                    orca.get_injectable("outputs_dir"),
                    "run%d_hazards_slr_buildings_%d.csv" % (run_number, year),
                )
            )


@orca.step()
def hazards_eq_summary(
    run_setup, run_number, year, households, jobs, parcels, buildings
):
    ...
    if run_setup["run_eq"]:
        if year == 2035:
            ...
            f = open(
                os.path.join(
                    orca.get_injectable("outputs_dir"),
                    "run%d_hazards_eq_%d.log" % (run_number, year),
                ),
                "w",
            )
            ...
            code_counts_df = pd.DataFrame(
                code_counts, columns=["building_code", "count"]
            )
            ...
            fragility_counts = [[x, fragilities.count(x)] for x in set(fragilities)]
            fragility_counts_df = pd.DataFrame(
                fragility_counts, columns=["fragility_code", "count"]
            )
            ...
            eq_demolish["count"] = 1
            eq_demolish = eq_demolish.drop(
                ["parcel_id", "year_built", "redfin_sale_year"], axis=1
            )
            eq_demolish = eq_demolish.groupby(["taz"]).sum()
            eq_demolish.to_csv(
                os.path.join(
                    orca.get_injectable("outputs_dir"),
                    "run%d_hazards_eq_demolish_buildings_%d.csv" % (run_number, year),
                )
            )
            ...
            if run_setup["eq_mitigation"]:
                ...
                retrofit_bldgs_tot_taz = misc.reindex(
                    parcels.zone_id, retrofit_bldgs_tot.parcel_id
                )
                ...
                retrofit_bldgs_tot.to_csv(
                    os.path.join(
                        orca.get_injectable("outputs_dir"),
                        "run%d_hazards_eq_retrofit_buildings_%d.csv"
                        % (run_number, year),
                    )
                )
        ...
        if year in [2030, 2035, 2050] and eq:
            buildings = buildings.to_frame()
            buildings_taz = misc.reindex(parcels.zone_id, buildings.parcel_id)
            ...
            buildings.to_csv(
                os.path.join(
                    orca.get_injectable("outputs_dir"),
                    "run%d_hazards_eq_buildings_list_%d.csv" % (run_number, year),
                )
            )
        ...
        if year == base_year:
            ...
            slack.chat.post_message(
                ...
                % (
                    ...
                    host,
                    devproj_len,
                    dropped_devproj_geomid,
                    dropped_devproj_proc,
                ),
                as_user=True,
            )
