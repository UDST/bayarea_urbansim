...
def transit_type(costar, parcels_geography):
    return (
        misc.reindex(parcels_geography.tpp_id, costar.parcel_id)
        .reindex(costar.index)
        .fillna("none")
    )
...
def vacant_job_spaces(buildings, jobs):
    ...
    return buildings.job_spaces.sub(s.value_counts(), fill_value=0).astype("int")


@orca.column("buildings")
def vacant_res_units(buildings, households):
    s = households.building_id[households.building_id != -1]
    return buildings.residential_units.sub(s.value_counts(), fill_value=0).astype("int")
...
def sqft_per_job(
    ...
    if run_setup["run_telecommute_strategy"] and year != base_year:
        sqft_per_job = sqft_per_job * superdistrict.map(
    ...
    # if the telecommute strategy is enabled, instead adjust future year sqft_per_job rates with the factor for that year
        ...
        sqft_per_job = sqft_per_job * superdistrict.map(
            sqft_per_job_adjusters["sqft_per_job_factor"]
        )
...
def transit_type(buildings, parcels_geography):
    return (
        misc.reindex(parcels_geography.tpp_id, buildings.parcel_id)
...
def sqft_per_unit(buildings):
    return (buildings.building_sqft / buildings.residential_units.replace(0, 1)).clip(
        400, 6000
    )
...
def modern_condo(buildings):
    ...
    return ((buildings.year_built > 2000) * (buildings.building_type == "HM")).astype(
        "int"
    )
...
def vmt_res_fees(parcels, run_setup):
    if run_setup["run_vmt_fee_res_for_res_strategy"]:
        ...
        res_fees = (
            parcels.vmt_res_cat.map(vmt_settings["res_for_res_fee_amounts"])
            if run_setup["run_vmt_fee_res_for_res_strategy"]
            else 0
        )
        return res_fees
...
def vmt_com_fees(parcels, run_setup):
    if (
        ...
        vmt_settings = account_strategies["acct_settings"]["vmt_settings"]
        com_for_res_fees = (
            parcels.vmt_nonres_cat.map(vmt_settings["com_for_res_fee_amounts"])
...
def parcel_average_price(use, quantile=0.5):
    if use == "residential":
        # get node price average and put it on parcels
        s = misc.reindex(
            orca.get_table("nodes")[use], orca.get_table("parcels").node_id
        )
...
def newest_building(parcels, buildings):
    return (
        buildings.year_built.groupby(buildings.parcel_id)
        .max()
        .reindex(parcels.index)
        .fillna(1800)
    )
...
def is_sanfran(parcels_geography, buildings, parcels):
    return (
        (parcels_geography.juris_name == "San Francisco")
        .reindex(parcels.index)
        .fillna(False)
        .astype("int")
    )
...
def nodev(zoning_existing, parcels, static_parcels):
    # nodev from zoning
    s1 = zoning_existing.nodev.reindex(parcels.index).fillna(0).astype("bool")
...
def built_far(parcels):
    ...
    s[parcels.parcel_acres < 0.1] = np.nan
    return s
...
def max_far(parcels_zoning_calculations, parcels, zoning_adjusters):
    ...
    if zoning_adjusters["dont_build_most_dense_building"]:
        ...
        s2 = misc.reindex(s2, parcels.zone_id).fillna(0.2)
        s = pd.concat([s, s2], axis=1).min(axis=1)
...
def max_dua(parcels_zoning_calculations, parcels, zoning_adjusters):
    ...
    # we had trouble with the zoning outside of the footprint
    # make sure we have rural zoning outside of the footprint
    s2 = parcels.urban_footprint.map({0: 0.01, 1: np.nan})
    s = pd.concat([s, s2], axis=1).min(axis=1)
...
def building_purchase_price(parcels):
    ...
    return (
        (parcels.total_sqft * parcels.building_purchase_price_sqft * 0.8 * 0.85)
        .reindex(parcels.index)
        .fillna(0)
    )
...
def cml(parcels, mandatory_accessibility, accessibilities_segmentation):
    mand_acc = mandatory_accessibility.local
    acc_seg = accessibilities_segmentation.local
    cols_to_sum = []
    for col in mand_acc.columns[
        ~mand_acc.columns.isin(["destChoiceAlt", "taz", "subzone", "weighted_sum"])
    ]:
        if col in acc_seg.columns:
            mand_acc[col] = (
                (mand_acc[col] - mand_acc[col].min()) / 0.0134
            ) * acc_seg.loc[0, col]
...
@orca.column("parcels", cache=True, cache_scope="iteration")
def cnml(parcels, non_mandatory_accessibility, accessibilities_segmentation):
    nmand_acc = non_mandatory_accessibility.local
    acc_seg = accessibilities_segmentation.local
    cols_to_sum = []
    for col in nmand_acc.columns[
        ~nmand_acc.columns.isin(["destChoiceAlt", "taz", "subzone", "weighted_sum"])
    ]:
        if col in acc_seg.columns:
            nmand_acc[col] = (
                (nmand_acc[col] - nmand_acc[col].min()) / 0.0175
            ) * acc_seg.loc[0, col]
            cols_to_sum.append(col)
...
def zone_cml(year, mandatory_accessibility, accessibilities_segmentation):
    mand_acc = mandatory_accessibility.local
    acc_seg = accessibilities_segmentation.local
...
@orca.column("zones", cache=True, cache_scope="iteration")
def zone_cnml(year, non_mandatory_accessibility, accessibilities_segmentation):
    nmand_acc = non_mandatory_accessibility.local
    acc_seg = accessibilities_segmentation.local
...
def ave_unit_sqft(buildings):
    return buildings.sqft_per_unit.groupby(buildings.zone_id).quantile(0.6)
...
def effective_max_dua(zoning_existing, parcels):
    ...
    max_far_from_height = (
        zoning_existing.max_height / HEIGHT_PER_STORY
    ) * PARCEL_USE_EFFICIENCY

    max_dua_from_height = max_far_from_height * 43560 / GROSS_AVE_UNIT_SIZE

    s = pd.concat(
        [zoning_existing.max_dua, max_dua_from_far, max_dua_from_height], axis=1
    ).min(axis=1)
...
def effective_max_office_far(parcels_zoning_calculations):
    return parcels_zoning_calculations.effective_max_far * parcel_is_allowed("office")
...
def zoned_du_underbuild(parcels, parcels_zoning_calculations):
    ...
    # 11 story building
    s = s[ratio > 0.5].reindex(parcels.index).fillna(0)
    return s.astype("int")
...
def zoned_far_build_ratio(parcels, parcels_zoning_calculations):
    ...
    s = parcels.total_non_residential_sqft / (
        parcels_zoning_calculations.effective_max_far * parcels.parcel_size
    )
    return s.replace(np.inf, 1).clip(0, 1)
