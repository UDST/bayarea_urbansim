...
def eq_code_buildings(buildings, year):
    ...
    if year == 2035:
        ...
        for i in buildings.index:
            if (
                buildings["building_type"][i] == "HS"
                and buildings["year_built"][i] <= 2015
            ):
                a = "SF"
                if buildings["stories"][i] == 1:
                    ...
                    if buildings["year_built"][i] <= 1940:
                        ...
                        buildings["year_built"][i] >= 1941
                        and buildings["year_built"][i] <= 1960
                    ):
                        c = "G2"
                    elif (
                        buildings["year_built"][i] >= 1961
                        and buildings["year_built"][i] <= 1995
                    ):
                        c = "G3"
                    elif (
                    ...
                    if buildings["year_built"][i] <= 1920:
                        ...
                        buildings["year_built"][i] >= 1921
                        and buildings["year_built"][i] <= 1940
                    ):
                        c = "G2"
                    elif (
                        buildings["year_built"][i] >= 1941
                ...
                buildings["building_type"][i] == "HM"
                or buildings["building_type"][i] == "MR"
            ) and buildings["year_built"][i] <= 2015:
                if (
                    buildings["residential_units"][i] == 2
                    or buildings["residential_units"][i] == 3
                    or buildings["residential_units"][i] == 4
                ):
                    a = "DU"  # 2, 3, & 4 units
                    ...
                    if buildings["stories"][i] >= 2:
                        ...
                        if buildings["year_built"][i] <= 1920:
                            ...
                            buildings["year_built"][i] >= 1961
                            and buildings["year_built"][i] <= 1977
                        ):
                            c = "G4"
                        elif (
                            buildings["year_built"][i] >= 1978
                            and buildings["year_built"][i] <= 1991
                        ):
                            c = "G5"
                        elif (
                            buildings["year_built"][i] >= 1992
                            and buildings["year_built"][i] <= 2015
                        ):
                            c = "G6"
                    ...
                    if buildings["stories"][i] == 1:
                        ...
                        if buildings["year_built"][i] <= 1920:
                            ...
                            c = "G5"
                    elif buildings["stories"][i] >= 2 and buildings["stories"][i] <= 5:
                        b = "25"
                        if buildings["year_built"][i] <= 1920:
                            c = "G1"
                        elif (
                        ...
                        if buildings["year_built"][i] <= 1950:
                            ...
                            buildings["year_built"][i] >= 1951
                            and buildings["year_built"][i] <= 1971
                        ):
                            c = "G2"
                        elif (
                            ...
                            buildings["year_built"][i] >= 1996
                            and buildings["year_built"][i] <= 2006
                        ):
                            c = "G4"
                        elif (
                            buildings["year_built"][i] >= 2007
                            and buildings["year_built"][i] <= 2015
                        ):
                            c = "G5"
                ...
                if buildings["year_built"][i] <= 1933:
                    ...
                    buildings["year_built"][i] >= 1934
                    and buildings["year_built"][i] <= 1950
                ):
                    c = "G2"
                elif (
                    buildings["year_built"][i] >= 1951
                    and buildings["year_built"][i] <= 1972
                ):
                    c = "G3"
                elif (
                    buildings["year_built"][i] >= 1973
                    and buildings["year_built"][i] <= 1996
                ):
                    c = "G4"
                elif (
                    buildings["year_built"][i] >= 1997
                    and buildings["year_built"][i] <= 2006
                ):
                    c = "G5"
                elif (
            ...
            if (
                ...
                fragility = 1.2
            elif code_i == "SF2PG4N" or code_i == "MF5PG3N" or code_i == "OTNNG4N":
                fragility = 1.3
                ...
                fragility = 2.5
            elif code_i == "MF25G2N" or code_i == "MF25G3N" or code_i == "MF25G4N":
                fragility = 3
            elif code_i == "NNNNNNN":
                fragility = 0
...
def earthquake_demolish(
    run_setup, parcels, buildings, households, jobs, residential_units, year
):
    ...
    if year == 2035:
        ...
        census_tract = pd.Series(parcels_tract["census_tract"], parcels_tract.index)
        ...
        parcels_tract = parcels_tract.to_frame(columns=["parcel_id", "census_tract"])
        parcels_tract = parcels_tract[["census_tract", "parcel_id"]]
        tract_parcels_grp = []
        tracts = []
        parcels_tract = sorted(parcels_tract.values, key=itemgetter(0))
        for tract, parcels in itertools.groupby(parcels_tract, key=itemgetter(0)):
        ...
        for i in range(len(tracts)):
            ...
            top_build_frag = build_frag[: int(round(len(build_frag) * existing_pct))]
            ...
            if run_setup["run_eq_mitigation"]:
                ...
                top_build_frag_bldgs = buildings[
                    buildings.index.isin(top_build_frag.index)
                ]
                retrofit_bldgs = top_build_frag_bldgs[
                    top_build_frag_bldgs.earthquake_code.isin(retrofit_codes)
                ]
                retro_no = int(round(float(len(retrofit_bldgs)) / 2))
                retrofit_set = np.random.choice(
                    retrofit_bldgs.index, retro_no, replace=False
                )
                # update top_build_frag to remove retrofit buildings
                top_build_frag = top_build_frag[
                    ~top_build_frag.index.isin(retrofit_set)
                ]
                # add table of retrofit buildings that weren't destroyed
                retrofit_bldgs_set = buildings[buildings.index.isin(retrofit_set)]
                retrofit_bldgs_tot = retrofit_bldgs_tot.append(retrofit_bldgs_set)
            ...
            if len(buildings_new) > 0:
                ...
                buildings_new_rand = np.random.choice(
                    buildings_new.index, new_no, replace=False
                )
                # add to a list of buildings to destroy
            ...
            buildings_i_remain = buildings_i[~buildings_i.index.isin(buildings_top)]
            if len(buildings_new) > 0:
            ...
            fire_buildings_rand = buildings_i_remain["fire_destroy"].sort_values(
                ascending=False
            )
            top_fire_buildings = fire_buildings_rand[
                : int(round(len(fire_buildings_rand) * fire_pct))
            ]
            # add to a list of buildings to destroy
        ...
        print("Total number of buildings being destroyed is: %d" % len(eq_buildings))
        ...
        hh_unplaced_eq = hh_unplaced_eq[~hh_unplaced_eq.index.isin(hh_unplaced.index)]
        orca.add_injectable("hh_unplaced_eq", hh_unplaced_eq)
        jobs = orca.get_table("jobs")
        jobs = jobs.to_frame()
        jobs_unplaced_eq = jobs[jobs["building_id"] == -1]
        jobs_unplaced_eq = jobs_unplaced_eq[
            ~jobs_unplaced_eq.index.isin(jobs_unplaced.index)
        ]
