...
def move_jobs_from_portola_to_san_mateo_county(parcels, buildings, jobs_df):
    ...
    juris = misc.reindex(parcels.juris, misc.reindex(buildings.parcel_id, jobs_df.building_id))
...
def preproc_households(store):
    ...
    df["base_income_quartile"] = pd.Series(pd.qcut(df.income, 4, labels=False), index=df.index).add(1)
    df["base_income_octile"] = pd.Series(pd.qcut(df.income, 8, labels=False), index=df.index).add(1)
    ...
    overrides = pd.read_csv(os.path.join(orca.get_injectable("inputs_dir"), "basis_inputs/edits/household_building_id_overrides.csv"),
