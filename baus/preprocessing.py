...
def allocate_jobs(baseyear_taz_controls, mapping, buildings, parcels):
    ...
    df = pd.DataFrame(jobs, columns=["sector_id", "empsix", "taz", "building_id"])
    ...
    for taz, cnt in df.groupby("taz").size().iteritems():
        ...
        potential_add_locations = buildings.non_residential_sqft[
            (zone_id == taz) & (buildings.non_residential_sqft > 0)
        ]

        if len(potential_add_locations) == 0:
            # if no non-res buildings, put jobs in res buildings
            potential_add_locations = buildings.building_sqft[zone_id == taz]
        ...
        if len(potential_add_locations) > 0:
            ...
            print(
                "ERROR in TAZ {}: {} jobs, {} potential locations".format(
...
def preproc_households(store):
    ...
    df["base_income_quartile"] = pd.Series(
        pd.qcut(df.income, 4, labels=False), index=df.index
    ).add(1)
    df["base_income_octile"] = pd.Series(
        pd.qcut(df.income, 8, labels=False), index=df.index
    ).add(1)
    ...
    overrides = pd.read_csv(
        os.path.join(
            ...
            "basis_inputs/edits/household_building_id_overrides.csv",
        ),
...
def assign_deed_restricted_units(df, parcels):
    ...
    zone_ids = misc.reindex(parcels.zone_id, df.parcel_id).reindex(df.index).fillna(-1)
    # sample deed restricted units to match current deed restricted unit
    # zone totals
    for taz, row in pd.read_csv(
        os.path.join(
            orca.get_injectable("inputs_dir"),
        ...
        potential_add_locations = df.residential_units[
            (zone_ids == taz) & (df.residential_units > 0)
        ]
    ...
    print(
        "Total deed restricted units after random selection: %d"
        % df.deed_restricted_units.sum()
    )

    df["deed_restricted_units"] = df[
        ["deed_restricted_units", "residential_units"]
    ].min(axis=1)

    print(
        "Total deed restricted units after truncating to res units: %d"
        % df.deed_restricted_units.sum()
    )
...
def correct_baseyear_vacancies(buildings, parcels, jobs, store):
    ...
    SURPLUS_VACANCY = pd.DataFrame(
        [SURPLUS_VACANCY_COUNTY, SURPLUS_VACANCY_JURIS]
    ).min()

    # count of jobs by building
    job_counts_by_building = (
        jobs.building_id.value_counts().reindex(buildings.index).fillna(0)
    )

    # with SURPLUS_VACANCY vacancy
    job_counts_by_building_surplus = (
        job_counts_by_building * (SURPLUS_VACANCY + 1)
    ).astype("int")

    # min of job spaces and vacancy
    correct_job_spaces = pd.DataFrame(
        [job_counts_by_building_surplus, buildings.job_spaces]
    ).min()
    ...
    s = (
        buildings.job_spaces.groupby(buildings_juris).sum() / jobs_juris.value_counts()
        - 1.0
    )
...
def preproc_buildings(store, parcels, manual_edits):
    ...
    df["residential_units"] = pd.concat(
        [df.residential_units, store.households_preproc.building_id.value_counts()],
        axis=1,
    ).max(axis=1)
    ...
    for index, row, col, val in edits[["id", "attribute", "new_value"]].itertuples():
    ...
    df["building_sqft"] = pd.DataFrame(
        {"one": df.building_sqft, "two": df.residential_sqft + df.non_residential_sqft}
    ).max(axis=1)
