...
from baus.utils import (
    ...
    groupby_random_choice,
    parcel_id_to_geom_id,
...
def static_parcels(developer_settings, parcels):
    ...
    return geom_id_to_parcel_id(
        pd.DataFrame(index=static_parcels), parcels
    ).index.values
...
def _proportional_jobs_model(
    ...
    current_jobs = jobs_df[jobs_df.empsix == sector][groupby_col].value_counts()
    ...
    available_jobs = jobs_df.query("empsix == '%s' and building_id == -1" % sector)
...
def accessory_units_strategy(run_setup, year, buildings, parcels, accessory_units):
    ...
    buildings.local.loc[
        add_buildings.index, "residential_units"
    ] += add_buildings.values


@orca.step()
def proportional_elcm(
    jobs,
    households,
    buildings,
    parcels,
    proportional_retail_jobs_forecast,
    ...
    df = orca.merge_tables(
        target="jobs", tables=[jobs, buildings, parcels], columns=["juris", "zone_id"]
    )
    ...
    buildings_df = buildings_df.rename(
        columns={"zone_id_x": "zone_id", "general_type_x": "general_type"}
    )
    ...
    mapping_d = {
        ...
        "County Pop": taz_assumptions_df.County.map(hh_df["county"].value_counts()),
        "Reg Pop": len(hh_df),
    }
    ...
    for col, mult in zip(taz_assumptions_df.columns, multipliers):
        target_jobs += (
            taz_assumptions_df[col].astype("float") * mapping_d[mult] * pop_to_hh
        ).fillna(0)
...
def jobs_relocation(
    ...
    run_setup,
    employment_relocation_rates_adjusters,
    years_per_iter,
    settings,
    static_parcels,
    buildings,
...
@orca.step()
def household_relocation(
    households, household_relocation_rates, run_setup, static_parcels, buildings
):
    ...
    if run_setup["run_renter_protections_strategy"]:
        renter_protections_relocation_rates = orca.get_table(
            "renter_protections_relocation_rates"
        )
        rates = pd.concat(
            [rates, renter_protections_relocation_rates.to_frame()]
        ).drop_duplicates(
    ...
    df = pd.merge(
        households.to_frame(["zone_id", "base_income_quartile", "tenure"]),
        rates,
    ...
    households.update_col_from_series(
        "building_id", pd.Series(-1, index=index), cast=True
    )
...
def scheduled_development_events(
    ...
    if year == (base_year + years_per_iter):
        demolish = demolish_events.to_frame().query(
            "%d <= year_built <= %d" % (year - years_per_iter, year)
        )
    else:
        demolish = demolish_events.to_frame().query(
            "%d < year_built <= %d" % (year - years_per_iter, year)
    ...
    orca.add_injectable(
        "static_parcels",
        np.append(
            static_parcels, demolish.loc[demolish.action == "build", "parcel_id"]
        ),
    )
    ...
    # also runs SDEM, eg 2015 pulls 2015-2014, while 2010 pulls 2010 projects
    if year == (base_year + years_per_iter):
        dps = development_projects.to_frame().query(
            "%d <= year_built <= %d" % (year - years_per_iter, year)
        )
    else:
        dps = development_projects.to_frame().query(
    ...
    new_buildings["form"] = new_buildings.building_type.map(
        mapping["building_type_map"]
    ).str.lower()
    new_buildings[
        "job_spaces"
    ] = new_buildings.non_residential_sqft / new_buildings.building_type.fillna(
        "OF"
    ).map(
        building_sqft_per_job
    )
    ...
    if (
        run_setup["run_vmt_fee_res_for_res_strategy"]
        or run_setup["run_sb_743_strategy"]
    ):
    ...
    if (
        run_setup["run_vmt_fee_com_for_com_strategy"]
        or run_setup["run_vmt_fee_com_for_res_strategy"]
    ):
        vmt_fee_categories = orca.get_table("vmt_fee_categories")
    ...
    new_buildings["pda_id"] = parcels_geography.pda_id.loc[
...
def add_extra_columns_func(df):
    ...
    if "deed_restricted_units" not in df.columns:
        ...
        print(
            "Number of deed restricted units built = %d"
    ...
    if "inclusionary_units" not in df.columns:
        ...
        print("Number of inclusionary units built = %d" % df.inclusionary_units.sum())

    if "subsidized_units" not in df.columns:
        df["subsidized_units"] = 0
    else:
        print("Number of subsidized units built = %d" % df.subsidized_units.sum())
...
@orca.step()
def alt_feasibility(
    parcels, developer_settings, parcel_sales_price_sqft_func, parcel_is_allowed_func
):
    ...
    config.building_efficiency = 0.85
    config.parcel_coverage = 0.85
    ...
    f = subsidies.policy_modifications_of_profit(
        orca.get_table("feasibility").to_frame(), parcels
    )
...
def residential_developer(
    ...
    num_units = dev.compute_units_to_build(
        len(households), buildings["residential_units"].sum(), target_vacancy
    )
    ...
    for parcel_mask, target, final_target, juris in targets:
        ...
        print("Running developer for %s with target of %d" % (str(juris), target))
        ...
        buildings = orca.get_table("buildings")
        print(
            "Stats of buildings before run_developer(): \n{}".format(
                buildings.to_frame()[
                    ["deed_restricted_units", "preserved_units", "inclusionary_units"]
                ].sum()
            )
        )
...
def retail_developer(
    ...
    all_units = dev.compute_units_to_build(
        len(jobs), buildings.job_spaces.sum(), dev_settings["kwargs"]["target_vacancy"]
    )
    ...
    print(
        "Building {:,} retail sqft in {:,} projects".format(
...
def office_developer(
    ...
    for typ in ["Office"]:
        ...
        if year > 2015 and typ in limits_settings:
            ...
            juris_name = parcels_geography.juris_name.reindex(parcels.index).fillna(
                "Other"
            )
            ...
            for juris, limit in limits_settings[typ].items():
                ...
                if target <= 0:
                    ...
                    print(
                        "    target = %d, current_total = %d" % (target, current_total)
                    )
                    continue
...
@orca.step()
def developer_reprocess(
    buildings, year, years_per_iter, jobs, parcels, summary, parcel_is_allowed_func
):
    ...
    to_add = res_units * 0.05 - job_spaces
    if to_add > 0:
        ...
        add_indexes = np.random.choice(
            res_units.index.values,
            size=to_add,
            replace=True,
    ...
    new_buildings.non_residential_sqft = (
        new_buildings.building_sqft / new_buildings.stories * 0.8
    )
    ...
    print(
        "Adding %d sqft of ground floor retail in %d locations"
    ...
    new_buildings["job_spaces"] = (new_buildings.non_residential_sqft / 445.0).astype(
        "int"
    )
    ...
    sqft_by_gtype = (
        buildings_df.query("year_built >= %d" % year)
...
def proportional_job_allocation(parcel_id):
    ...
    buildings = (
        ...
        .to_frame(["parcel_id", "job_spaces", "zone_id", "year_built"])
    ...
    for index, building in buildings.query("year_built > 2015").iterrows():
        ...
        sectors = np.random.choice(
            job_dist.index, size=num_new_jobs, p=job_dist / job_dist.sum()
        )
        ...
        print(
            "Adding {} new jobs to parcel {} with proportional model".format(
...
def make_network(name, weight_col, max_distance):
    ...
    net = pdna.Network(
        nodes["x"], nodes["y"], edges["from"], edges["to"], edges[[weight_col]]
    )
...
def make_network_from_settings(settings):
    return make_network(
        settings["name"], settings.get("weight_col", "weight"), settings["max_distance"]
    )
...
def local_pois(accessibility_settings):
    ...
    # preprocessing step
    n = make_network(
        accessibility_settings["build_networks"]["walk"]["name"], "weight", 3000
    )
    ...
    locations = pd.read_csv(
        os.path.join(
            orca.get_injectable("inputs_dir"), "accessibility/pandana/bart_stations.csv"
        )
    )
...
def regional_vars(net):
    ...
    nodes2 = pd.read_csv(
        os.path.join(
            ...
            "accessibility/pandana/regional_poi_distances.csv",
        ),
        index_col="tmnode_id",
    )
...
def regional_pois(accessibility_settings, landmarks):
    ...
    # preprocessing step
    n = make_network(
        accessibility_settings["build_networks"]["drive"]["name"], "CTIMEV", 75
    )
