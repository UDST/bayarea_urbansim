...
for year in range(1):
    ...
    if retval:
        open(os.path.join(misc.output_dir(), "rrh_estimate.json"), "w").write(simplejson.dumps(retval, sort_keys=True, indent=4))
    ...
    if retval:
        open(os.path.join(misc.output_dir(), "hlcmr_estimate.json"), "w").write(simplejson.dumps(retval, sort_keys=True, indent=4))
