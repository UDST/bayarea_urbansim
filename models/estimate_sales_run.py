...
for year in range(1):
    ...
    if retval:
        open(os.path.join(misc.output_dir(), "rsh_estimate.json"), "w").write(simplejson.dumps(retval, sort_keys=True, indent=4))
    ...
    if retval:
        open(os.path.join(misc.output_dir(), "hlcms_estimate.json"), "w").write(simplejson.dumps(retval, sort_keys=True, indent=4))
