...
for year in range(3): 
  ...
  if retval: open(os.path.join(misc.output_dir(),"hhlds_run.json"),"w").write(simplejson.dumps(retval,sort_keys=True,indent=4))
  print "Running feasibility_run"
  import feasibility_run
  retval = feasibility_run.feasibility_run(dset,year=2010+year)
  if retval: open(os.path.join(misc.output_dir(),"feasibility_run.json"),"w").write(simplejson.dumps(retval,sort_keys=True,indent=4))
